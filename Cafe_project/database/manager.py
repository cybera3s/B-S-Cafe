...
class DBManager:
    ...
    def create(self, model_instance: DBModel) -> int:
        with self.conn:
            ...
            model_vars = vars(
                model_instance
            )  # get model variables as a dict ->{'first_name':"akbar", 'last_name':...}
            model_fields_str = ",".join(
                model_vars.keys()
            )  # get field names in string format->first_name, last_name,..
            model_values_str = ",".join(
                ["%s"] * len(model_vars)
            ...
            with curs:
                ...
                )  # set ID into the input model_instance
                return id
    ...
    def read(self, model_class: type, pk) -> DBModel:  # get
        ...
        with self.conn:
            ...
            with curs:
                curs.execute(
                    f"""SELECT * FROM {model_class.TABLE} WHERE {model_class.PK} = {pk}"""
                )
                res = curs.fetchone()
                return model_class(
                    **dict(res)
    ...
    def update(self, model_instance: DBModel) -> None:
        ...
        with self.conn:
            ...
            with curs:
                ...
                model_pk_value = getattr(
                    model_instance, model_instance.PK
                )  # value of pk (for ex. 'id' in patient)
                model_set_values = [
                    f"{field} = %s" for field in model_vars
                ]  # -> ['first_name=%s', 'last_name'=%s,...]
                model_values_tuple = tuple(model_vars.values())
                curs.execute(
                    f"""UPDATE {model_instance.TABLE} SET {','.join(model_set_values)}
                 WHERE {model_instance.PK} = {model_pk_value};""",
                    model_values_tuple,
                )
    ...
    def delete(self, model_instance: DBModel) -> None:
        ...
        with self.conn:
            ...
            with curs:
                ...
                curs.execute(
                    f"""DELETE FROM {model_instance.TABLE} WHERE {model_instance.PK} = {model_pk_value};"""
                )
                delattr(
                    model_instance, "id"
                )  # deleting attribute 'id' from the deleted instance
    ...
    def read_by(self, model_class: type, column: tuple) -> list:
        ...
        with self.conn:
            ...
            with curs:
                curs.execute(
                    f"""SELECT * FROM {model_class.TABLE} WHERE {key} = '{value}';"""
                )
                res = list(map(dict, curs.fetchall()))
                res = [model_class(**item) for item in res]
                return res
